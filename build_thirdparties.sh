...
emconfigure $source_path/libraw/configure --disable-examples --disable-jasper CFLAGS="-fPIC" CXXFLAGS="-fPIC"
emmake make "${MAKEFLAGS}"

echo "Building libheif"
mkdir -p $build_path/libheif
cd $build_path/libheif
CONFIGURE_ARGS="-DENABLE_MULTITHREADING_SUPPORT=OFF -DWITH_GDK_PIXBUF=OFF -DWITH_EXAMPLES=OFF -DBUILD_SHARED_LIBS=OFF -DENABLE_PLUGIN_LOADING=OFF -DWITH_LIBDE265=OFF -DBUILD_TESTING=OFF"

emcmake cmake $source_path/libheif  $CONFIGURE_ARGS $CMAKE_BUILD_TYPE
emmake make "${MAKEFLAGS}"
